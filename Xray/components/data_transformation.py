...
class DataTransformation:
    ...
    def transforming_test_data(self) -> transforms.Compose:
        try:
            logging.info("Entered the transforming_test_data method of DataTransformation class")
            ...
            logging.info("Exited the transforming_test_data method of DataTransformation class")
    ...
    def data_loader(self,train_transform:transforms.Compose,test_transform:transforms.Compose) -> Tuple[DataLoader,DataLoader]:
        try:
            logging.info("Entered the data_loader method of DataTransformation class")
            
            train_data: Dataset = ImageFolder(os.path.join(self.data_ingestion_artifact.train_data_path),
                                              transform=train_transform)
            test_data: Dataset = ImageFolder(os.path.join(self.data_ingestion_artifact.test_data_path),
                                             transform=test_transform)
            
            logging.info("Created train and test dataset paths")
            
            train_loader: DataLoader = DataLoader(dataset=train_data,
                                                  **self.data_transformation_config.data_loader_params)
            test_loader: DataLoader = DataLoader(dataset=test_data,
                                                 **self.data_transformation_config.data_loader_params)
